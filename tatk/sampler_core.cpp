...
#pragma omp parallel for
        ...
        // void remove_edges(std::vector<EdgeIDType> &_srcs, std::vector<EdgeIDType> &_dsts, std::vector<EdgeIDType> &_eids, std::vector<TimeStampType> &_tss)
        // {
        //     std::vector<EdgeIDType> idx_fix(indptr.size(), 0);
        //     std::vector<EdgeIDType> idx_rmvs;
        //     int id;
        //     int idx_start;
        //     int idx_end;
        //     int idx_rmv;
        ...
        //     {
        //         id = _eids[i];
        //         idx_start = indptr[_srcs[i]];
        //         idx_end = indptr[_srcs[i]];
        //         j = 0;
        //         for (auto it = eid.begin() + idx_start; it < eid.begin() + idx_end; it++) 
        //         {
        //             if (*it == id) 
        //             {
        //                 break;
        ...
        //         }
        //         idx_rmv = idx_start + j;
        //         idx_rmvs.push_back(idx_rmv);
        //         for (int k=_srcs[i]+1; k < idx_fix.size(); k++) {
        //             idx_fix[k] += 1;
        //         }

        //         idx_start = indptr[_dsts[i]];
        //         idx_end = indptr[_dsts[i]];
        //         j = 0;
        //         for (auto it = eid.begin() + idx_start; it < eid.begin() + idx_end; it++) 
        //         {
        //             if (*it == id) 
        ...
        //         idx_rmv = idx_start + j;
        //         idx_rmvs.push_back(idx_rmv);
        //         for (int k=_dsts[i]+1; k < idx_fix.size(); k++) {
        //             idx_fix[k] += 1;
        //         }
        //     }
        //     std::sort(idx_rmvs.begin(), idx_rmvs.end());
        //     for (int i=0; i < indptr.size(); i++)
        //     {
        //         indptr[i] -= idx_fix[i];
        //     }
        //     for (int i=idx_rmvs.size()-1; i >= 0; i--)
        //     {
        //         indices.erase(indices.begin() + idx_rmvs[i]);
        //         eid.erase(eid.begin() + idx_rmvs[i]);
        //         ts.erase(ts.begin() + idx_rmvs[i]);
        //     }
        // }

        void update_dataset(std::vector<EdgeIDType> &_indptr, std::vector<EdgeIDType> &_indices,
                            std::vector<EdgeIDType> &_eid, std::vector<TimeStampType> &_ts)
        {
            indptr.assign(_indptr.begin(), _indptr.end());
            indices.assign(_indices.begin(), _indices.end());
            eid.assign(_eid.begin(), _eid.end());
            ts.assign(_ts.begin(), _ts.end());
            num_edges = indices.size();
        }
...
PYBIND11_MODULE(sampler_core, m)
{
    ...
    py::class_<ParallelSampler>(m, "ParallelSampler")
        ...
        // .def("remove_edges", &ParallelSampler::remove_edges)
        .def("update_dataset", &ParallelSampler::update_dataset)
