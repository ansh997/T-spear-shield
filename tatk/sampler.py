...
class NegLinkSamplerDST:
    ...
    def sample_v2(self, positives, neg_samples=1):
        min_neg_samples = min(len(self.dsts) - 1, neg_samples)
        negs = []
        for _, dst in enumerate(positives):
            p = (1 / (len(self.dsts) - 1)) * np.ones(len(self.dsts))
            p[self.dst_to_idx[dst]] = 0
            neg = self.gen.choice(self.dsts, min_neg_samples, replace=False, p=p)
            negs.append(neg)
        return np.concatenate(negs)
    
    def sample_v3(self, positives):
        negs = []
        for dst in positives:
            idx = self.dst_to_idx[dst]
            negs.append(np.delete(self.dsts, idx))
        return np.concatenate(negs)
    
    def sample_v4(self, positives, neg_samples=1):
        poss = np.tile(positives.reshape(-1, 1), neg_samples).reshape(1, -1).squeeze()
        negs = self.gen.choice(self.dsts, len(positives) * neg_samples)
        same = poss == negs
        while same.sum() != 0:
            neg = self.gen.choice(self.dsts, same.sum())
            negs[same] = neg
            same = poss == negs
        return negs
