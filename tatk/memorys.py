...
class EmbeddingBox():
    def __init__(self, num_nodes, dim_emb):
        self.dim_emb = dim_emb
        self.node_embedding = torch.zeros((num_nodes, dim_emb), dtype=torch.float32)
        self.node_embedding_ts = torch.zeros(num_nodes, dtype=torch.float32)
        self.node_activated = torch.zeros(num_nodes, dtype=torch.bool)
        self.device = torch.device('cpu')

    def reset(self):
        self.node_embedding.fill_(0)
        self.node_embedding_ts.fill_(0)
        self.node_activated.fill_(0)

    def move_to_gpu(self):
        self.node_embedding = self.node_embedding.cuda()
        self.node_embedding_ts = self.node_embedding_ts.cuda()
        self.node_activated = self.node_activated.cuda()
        self.device = torch.device('cuda:0')

    def update_embedding(self, nid, embedding, root_nodes, ts, neg_samples=1):
        if nid is None:
            return
        num_true_src_dst = root_nodes.shape[0] // (neg_samples + 2) * 2
        with torch.no_grad():
            nid = nid[:num_true_src_dst].to(self.device)
            embedding = embedding[:num_true_src_dst].to(self.device)
            ts = ts[:num_true_src_dst].to(self.device)
            self.node_embedding[nid.long()] = embedding
            self.node_embedding_ts[nid.long()] = ts
            self.node_activated[nid.long()] = True
    ...
    def prep_input_mails_cpu(self, mfg, use_pinned_buffers=False):
        for i, b in enumerate(mfg):
            if use_pinned_buffers:
                ...
                b.srcdata['mem'] = self.node_memory[b.srcdata['ID'].long()].cpu()
                b.srcdata['mem_ts'] = self.node_memory_ts[b.srcdata['ID'].long()].cpu()
                b.srcdata['mem_input'] = self.mailbox[b.srcdata['ID'].long()].cpu().reshape(b.srcdata['ID'].shape[0], -1)
                b.srcdata['mail_ts'] = self.mailbox_ts[b.srcdata['ID'].long()].cpu()
